...
contract(
  ...
  ([deployer, owner, other, ownerToken, defaultFeeReceiver, royaltiesRecipient, orderBook]) => {
    ...
    describe('Upgrade RoyaltiesRegistry', () => {
      it('Should upgrade royalties registry contract', async () => {
        const newRoyaltiesRegistry = await RoyaltiesRegistry.new();

        await royaltiesRegistryProxy.upgradeToAndCall(newRoyaltiesRegistry.address, '0x', {
          from: deployer,
        });
      });
      it('Variables should not change after upgrade', async () => {
        ...
        assert.equal(owner, await royaltiesRegistryProxy.owner());
      });
      it('Should not upgrade if not called by upgrade manager', async () => {
        const newRoyaltiesRegistry = await RoyaltiesRegistry.new();

        await truffleAssert.reverts(
          royaltiesRegistryProxy.upgradeToAndCall(newRoyaltiesRegistry.address, '0x', {
            from: other,
          }),
      ...
      it('Should change upgrade manager by owner', async () => {
        await royaltiesRegistryProxy.setUpgradeManager(other, { from: owner });

        assert.equal(await royaltiesRegistryProxy.upgradeManager(), other);
      });
    });
  },
);
